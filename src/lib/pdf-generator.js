...
// PDF Generator
// =============================================
// This file creates beautiful PDF course documents
// using the 'pdf-lib' library.
//
// Each PDF has:
// 1. A Cover Page (with title, category, author)
// 2. Chapter Pages (with headings, content, code snippets, takeaways)
//
// The generated PDFs are used as demo course content.

/**
 * Create a demo course PDF document.
 * 
 * @param {Object} guideContent - The content for the PDF
 * @param {string} guideContent.title - Course title
 * @param {string} guideContent.category - Course category (e.g., "Web Development")
 * @param {string} guideContent.author - Author/publisher name
 * @param {Array} guideContent.pages - Array of chapter page objects
 * @returns {Promise<Buffer>} - The generated PDF as a Buffer (binary data)
 */
export async function createDemoCoursePdf(guideContent) {
  // Create a new empty PDF document
  const pdfDoc = await PDFDocument.create();

  // Load standard fonts (built into PDF format, no external files needed)
  const fontRegular = await pdfDoc.embedFont(StandardFonts.Helvetica);
  const fontBold = await pdfDoc.embedFont(StandardFonts.HelveticaBold);
  const fontMono = await pdfDoc.embedFont(StandardFonts.Courier); // For code snippets
  ...
  // Draw a colored header bar at the top (brand indigo color)
  ...
  // Draw a small category badge (golden color)
  coverPage.drawRectangle({
    x: 40,
    y: PAGE_HEIGHT - 260,
    width: 120,
  ...
  // Write the category name on the badge
  ...
  // Write the main title on the cover
  ...
  coverPage.drawText(`Comprehensive Developer Masterclass & Practical Guide`, {
    ...
    size: 13,
    font: fontRegular,
    color: rgb(0.85, 0.9, 1), // Light blue text
  });

  // Write author name at the bottom
  ...
  // ==========================================
  // 2. CREATE CHAPTER / CONTENT PAGES
  // ==========================================
  for (let i = 0; i < guideContent.pages.length; i++) {
    ...
    page.drawRectangle({
      ...
      width: PAGE_WIDTH - 80,
      height: 1,
      color: rgb(0.85, 0.88, 0.92), // Light gray line
    });

    // Course title at top left
    ...
    // --- Chapter Heading ---
    let currentY = PAGE_HEIGHT - 90; // Track vertical position on page
    ...
    for (const p of pageData.content) {
      ...
      // Estimate how many lines this paragraph takes
      const lines = Math.ceil(p.length / 75);
      currentY -= lines * 16 + 12;
    }

    // --- Code Snippet Box (dark background, green text) ---
    if (pageData.codeSnippet && pageData.codeSnippet.length > 0) {
      ...
      page.drawRectangle({
        ...
        color: rgb(0.08, 0.1, 0.16), // Very dark (like a code editor)
      });
      ...
      for (const codeLine of pageData.codeSnippet) {
        page.drawText(codeLine, {
          ...
          y: codeY,
          size: 9.5,
          font: fontMono, // Monospace font for code
          color: rgb(0.55, 0.85, 0.6), // Green text (like terminal)
        });
        codeY -= 15;
      }
    ...
    if (pageData.takeaways && pageData.takeaways.length > 0) {
      ...
      page.drawRectangle({
        ...
        borderColor: rgb(0.3, 0.5, 0.9),  // Blue border
        borderWidth: 1,
      });
      ...
      // List each takeaway with bullet point
  ...
  // Save the PDF and convert to Buffer
