...
import { createDemoCoursePdf, GuidePdfContent } from './pdf-generator';
...
export async function seedDatabase() {
  ...
  if (!settings) {
    settings = await Settings.create({
      ...
      pointsEarnRate: 10, // 10% earned as points
  ...
  for (const course of sampleCoursesData) {
    ...
    if (!existingGuide) {
      ...
      // Ensure file exists and update fileKey if needed
  ...
  // 6. Pre-enroll Student into the first guide so student dashboard has immediate access to read!
